...
async function loadAllRaces() {
  ...
  allRacesCache = await prisma.race.findMany({
    ...
    orderBy: { date: 'asc' },
    // trackCondition included automatically (scalar field)
  })
...
async function main() {
  ...
  const steps = [0.05, 0.10, 0.15, -0.05, -0.10, -0.15]
  ...
  while (improved && iteration < 5) {
    ...
    for (const factor of factors) {
      ...
      for (const step of steps) {
        ...
        if (result.accuracy > localBest + 0.1) {  // 0.1%以上の改善が必要
